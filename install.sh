...
function fix_redis-server() {
    apt-get install -y redis-server || sed -i 's%ReadWriteDirectories=-/var/run/redis%ReadWriteDirectories=-/run/redis%' /lib/systemd/system/redis-server.service
